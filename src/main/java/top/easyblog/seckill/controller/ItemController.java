...
public class ItemController extends BaseController {
    ...
    public CommonResponse<ItemVO> createItem(@RequestParam(name = "title") String title,
        ...
        String itemCacheKey = String.format("%s%d", RedisKeyManager.ITEM_KEY_PREFIX, itemModelForReturn.getId());
        redisService.delete(RedisService.RedisDataBaseSelector.DB_0,RedisKeyManager.ITEM_LISTS_CACHE);
        redisService.set(itemCacheKey, JSON.toJSONString(itemModel), 600, RedisService.RedisDataBaseSelector.DB_0);
    ...
    public CommonResponse<ItemVO> getItem(@RequestParam(name = "id") Integer id) {
        String itemCacheKey = String.format("%s%d", RedisKeyManager.ITEM_KEY_PREFIX, id);
        ...
        if(itemModel==null) {
            ...
            //先从redis中查询对应商品的信息
            itemModel = JSON.parseObject((String) redisService.get(itemCacheKey, RedisService.RedisDataBaseSelector.DB_0), ItemModel.class);
            if (itemModel == null) {
                //redis中没有对应商品信息，就从数据库查询
                itemModel = itemService.getItemById(id);
                //将商品信息存放到hash中 有效时间10分钟
                redisService.set(itemCacheKey, JSON.toJSONString(itemModel),1800, RedisService.RedisDataBaseSelector.DB_0);
            }
    ...
    public CommonResponse<List<ItemVO>> listItem() {
        ...
        //首先从本地缓存中查询数据
        JSONArray jsonArray = (JSONArray)localCacheService.get(RedisKeyManager.ITEM_LISTS_CACHE);
        if(!StringUtils.isEmpty(jsonArray)){
            itemModelList=JSON.parseArray(jsonArray.toString(),ItemModel.class);
        }
        if(itemModelList==null) {
            //本地缓存没有去Redis缓存中查询
            String jsonStr = (String)redisService.get(RedisKeyManager.ITEM_LISTS_CACHE, RedisService.RedisDataBaseSelector.DB_0);
            ...
            if(itemModelList==null){
                ...
                redisService.set(RedisKeyManager.ITEM_LISTS_CACHE, JSON.toJSONString(itemModelList),1800, RedisService.RedisDataBaseSelector.DB_0);
            }
            localCacheService.set(RedisKeyManager.ITEM_LISTS_CACHE, (JSONArray)JSON.toJSON(itemModelList));
        }
